...
export class Radar {
  ...
  draw({ mapData, ship }) {
    ...
    // Auto-scale based on outermost planet orbit.
    let maxR = 40; // floor so empty systems still look reasonable
    for (const p of planets) {
      maxR = Math.max(maxR, dist2d(star.pos, p.pos));
    }
    maxR *= 1.3;
    ...
    // Orbit rings — faint amber concentrics, one per planet.
    ...
    for (const p of planets) {
      ...
      ctx.fill();
      // Soft outline for legibility against orbit ring
      ctx.strokeStyle = 'rgba(10, 12, 18, 0.8)';
      ctx.lineWidth = 0.8;
      ctx.stroke();
    }

    // Ship — triangle at world position, rotated to match yaw.
    // Clamp to edge if the player strayed outside the chart bounds.
    ...
    ctx.save();
    ctx.translate(sx, sz);
    // Three.js yaw=0 faces -Z (screen up). Canvas rotate(+θ) maps (0,-1) to
    // (sin θ, -cos θ), which rotates clockwise on screen. To match world yaw
    // (counterclockwise around +Y looking down), negate.
